...
class playersim:
    ...
    def __init__(self, pcards, pot, stacksize, betsize):
        self.suits = [0, 1, 2, 3]
    ...
    def initialize(self):
        ...
        while ( int(round(time.time() * 1000)) - starttime < 10000 ):
            values = [x+y for x,y in zip(self.decision(self.pcards), values)]
    ...
    def decision(self, pcards):
        ...
        for bet in self.bets:
            gsim = gamesim(self.bp, self.bp1, self.deck, self.stacksize, 500, self.pot, self.betsize, [], 0, bet, 20)
            values.append( round(gsim.run(), 2) )
        return values


if __name__ == '__main__':
    p = playersim([18, 33], 20, 500, 10)
